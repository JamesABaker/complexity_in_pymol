...
# Script overview
# The script takes a list of PDB IDs. It opens or downloads the PDB file.
# It then checks if the PDBTM database contains TMD boundary information.
# Phobius is also used as a backup. If phobius fails, and there is no PDBTM
# entry, then manual co-ordinates can be entered. After that, the TMHs are ran through TMSOC.
# The PDB file is copied and modified so that the B-factor column reflects
# the TMH complexity. All the information and logs are then added to a
# database.


# First, we try to parse the xml database for PDBTM.
tree = ET.parse('pdbtmalpha5-4-2017_CLEANED.xml')
root = tree.getroot()
print(root.attrib)
...
# This TMH list can be updated with any list of PDB files.
                        ...
                        if "TRANSMEM" in line:
                            ...
                            for item in line.split(" "):
                                ...
                                try:
                                    ...
                                    pass
                            # Now we check that there is only a start and a
                            # stop.
